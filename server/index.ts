...
import { applySchemaIfNeeded, pingDb } from './engine/db'
import {
  checkUpdate,
  clearNotifications,
  listLessonCatalog,
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`[alpha-code] listening on :${PORT}`)
  void applySchemaIfNeeded().catch((err) => {
    console.error('[DB] falha ao aplicar schema:', err?.message ?? err)
  })
})
